  ...
  <div class="max-w-4xl space-y-8">
    ...
    <div class="flex items-center justify-between">
      <div class="flex flex-col gap-1">
        <h1 class="text-3xl font-bold text-slate-900 dark:text-white tracking-tight">Notifications</h1>
    ...
    <div class="space-y-4">
      <div 
        ...
        :key="note.id"
        class="group bg-white dark:bg-[#002d39] rounded-3xl border border-slate-100 dark:border-white/5 p-6 flex items-start justify-between hover:shadow-lg hover:shadow-slate-200/50 dark:hover:shadow-black/20 transition-all duration-300 cursor-pointer"
      >
        <div class="flex gap-6">
          <div :class="['h-14 w-14 shrink-0 rounded-2xl flex items-center justify-center transition-transform group-hover:scale-110 duration-300', note.bg, note.color]">
            <component :is="note.icon" :size="28" />
          </div>
          <div class="space-y-1">
            <div class="flex items-center gap-3">
              <h3 class="text-lg font-bold text-slate-900 dark:text-white">{{ note.title }}</h3>
              <span class="text-[10px] font-bold text-slate-300 uppercase">{{ note.time }}</span>
            </div>
            <p class="text-sm text-slate-500 dark:text-white/40 leading-relaxed max-w-lg">{{ note.description }}</p>
          </div>
        </div>
        <button class="p-2 text-slate-300 dark:text-white/10 hover:text-slate-600 dark:hover:text-white">
