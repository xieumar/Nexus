  ...
  <div class="space-y-8">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
      <div class="flex flex-col gap-1">
        <h1 class="text-3xl font-bold text-slate-900 dark:text-white tracking-tight">System Logs</h1>
        <p class="text-slate-500 dark:text-white/40 text-sm font-medium">Real-time audit trail and event stream.</p>
      </div>
      <div class="flex items-center gap-3">
        <button class="flex items-center gap-2 px-4 py-2.5 rounded-xl bg-white dark:bg-[#002d39] border border-slate-200 dark:border-white/5 text-slate-600 dark:text-white/60 text-xs font-bold hover:bg-slate-50 dark:hover:bg-white/10 transition-all">
          <Download :size="16" />
          Export CSV
        </button>
        <button class="flex items-center gap-2 px-4 py-2.5 rounded-xl bg-red-50 dark:bg-red-500/10 text-red-600 dark:text-red-400 text-xs font-bold hover:bg-red-100 dark:hover:bg-red-500/20 transition-all">
          <Trash2 :size="16" />
          Clear Logs
        </button>
      </div>
        ...
        <table class="w-full text-left">
          ...
          <tbody class="divide-y divide-slate-50 dark:divide-white/5">
            <tr v-for="event in filteredEvents" :key="event.id" class="hover:bg-slate-50 dark:hover:bg-white/5 transition-colors">
              ...
              <td class="px-6 py-4 text-xs text-slate-600 dark:text-white/50">
                {{ event.message }}
              </td>
            </tr>
